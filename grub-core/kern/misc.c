...
#ifndef GRUB_UTIL
static void
private_backtrace_print_address (void *addr)
{
  grub_dl_t mod;
...
static int
private_unwind_frame (unsigned long *fp, unsigned long *sp, unsigned long *pc)
{
...
static void
private_backtrace (void)
{
  register unsigned long current_sp asm("sp");
  unsigned long fp = (unsigned long)__builtin_frame_address(0);
  unsigned long pc = (unsigned long)private_backtrace;
  unsigned long sp = current_sp;

  while (1)
    {
      grub_printf ("sp: %p fp: %p pc: ", (void *)sp, (void *)fp);
      private_backtrace_print_address ((void *)pc);
      grub_printf ("\n");

      if (private_unwind_frame (&fp, &sp, &pc) < 0)
	break;
    }
}
#endif
